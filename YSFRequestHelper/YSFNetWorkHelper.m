...
@implementation YSFNetWorkHelper
...
#pragma mark - 开始监听网络
+ (void)networkStatusWithBlock:(YSFNetworkStatus)networkStatus {
    [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            case AFNetworkReachabilityStatusUnknown:
            networkStatus ? networkStatus(YSFNetworkStatusUnknown) : nil;
            if (_isOpenLog) YSFLog(@"未知网络");
            break;
            case AFNetworkReachabilityStatusNotReachable:
            networkStatus ? networkStatus(YSFNetworkStatusNotReachable) : nil;
            if (_isOpenLog) YSFLog(@"无网络");
            break;
            case AFNetworkReachabilityStatusReachableViaWWAN:
            networkStatus ? networkStatus(YSFNetworkStatusReachableViaWWAN) : nil;
            if (_isOpenLog) YSFLog(@"手机自带网络");
            break;
            case AFNetworkReachabilityStatusReachableViaWiFi:
            networkStatus ? networkStatus(YSFNetworkStatusReachableViaWiFi) : nil;
            if (_isOpenLog) YSFLog(@"WIFI");
            break;
        }
    }];
}
...
+ (void)showRequestNetworkingSetting{
    NSString *title = YSFLocalizedReplacingString(@"允许\"xxxxx\"使用数据?", @"xxxxx", YSFLocalizedString(@"BLOCKSTAR"));
    NSString *message = YSFLocalizedString(@"当前网络连接失败，请跳转到设置页面，设置当前网络环境,可能同时包含无线局域网和蜂窝移动数据。");
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:title message:message preferredStyle:UIAlertControllerStyleAlert];
    [alertController addAction:[UIAlertAction actionWithTitle:YSFLocalizedString(@"不允许") style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        
    }]];
    
    [alertController addAction:[UIAlertAction actionWithTitle:YSFLocalizedString(@"允许") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
+ (NSURLSessionTask *)GET:(NSString *)URL
               parameters:(id)parameters
                  success:(YSFHttpRequestSuccess)success
                  failure:(YSFHttpRequestFailed)failure {
...
+ (NSURLSessionTask *)GET:(NSString *)URL
               parameters:(id)parameters
            responseCache:(YSFHttpRequestCache)responseCache
                  success:(YSFHttpRequestSuccess)success
                  failure:(YSFHttpRequestFailed)failure {
    ...
    {
        //读取缓存
        responseCache!=nil ? responseCache([YSFNetworkCache httpCacheForURL:URL parameters:parameters]) : nil;
    }
    NSURLSessionTask *sessionTask = [_sessionManager GET:URL parameters:parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (_isOpenLog) {
//            YSFLog(@"header = %@",task.currentRequest.allHTTPHeaderFields);
//            YSFLog(@"responseObject = %@",responseObject);
        }
        [[self allSessionTask] removeObject:task];
        success ? success(responseObject) : nil;
        //对数据进行异步缓存
        responseCache!=nil ? [YSFNetworkCache setHttpCache:responseObject URL:URL parameters:parameters] : nil;
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        if (_isOpenLog) {YSFLog(@"error = %@",error);}
        [[self allSessionTask] removeObject:task];
...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    if (_isOpenLog) {YSFLog(@"url = %@,param = %@",URL,parameters);}
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        
    } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
//        if (_isOpenLog) {YSFLog(@"responseObject = %@",responseObject);}
        [[self allSessionTask] removeObject:task];
        success ? success(responseObject) : nil;
        //对数据进行异步缓存
        responseCache!=nil ? [YSFNetworkCache setHttpCache:responseObject URL:URL parameters:parameters] : nil;
...
+ (NSURLSessionTask *)uploadFileWithURL:(NSString *)URL
                             parameters:(id)parameters
                               ...
                               progress:(YSFHttpProgress)progress
                                success:(YSFHttpRequestSuccess)success
                                failure:(YSFHttpRequestFailed)failure {
...
//    {
//        YSFLog(@"userInfo %@",[XQLoginExample lastUserInfo]);
//        //  如果登录了就传登录信息
...
+ (void)setRequestSerializer:(YSFRequestSerializer)requestSerializer {
    _sessionManager.requestSerializer = requestSerializer==YSFRequestSerializerHTTP ? [AFHTTPRequestSerializer serializer] : [AFJSONRequestSerializer serializer];
    _sessionManager.responseSerializer.acceptableContentTypes = kAcceptableContentTypes;
}

+ (void)setResponseSerializer:(YSFResponseSerializer)responseSerializer {
    _sessionManager.responseSerializer = responseSerializer==YSFResponseSerializerHTTP ? [AFHTTPResponseSerializer serializer] : [AFJSONResponseSerializer serializer];
}
...
@implementation NSDictionary (YSF)
